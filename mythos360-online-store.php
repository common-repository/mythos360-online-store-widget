...
class iframe_settings_config { 
	...
	//  DISPLAY SETTINGS
	var $title = "My Mythos360 Online Store";  
	var $intro_text = 'This page allows you to generate the required HTML code to place in any widget or page in your Wordpress to show your Mythos360 store Esta p&aacute;gina te permite configurar y generar el c&oacute;digo HTML que podr&aacute;s pegar en cualquier widget o p&aacute;gina de tu Wordpress para mostrar tu propia tienda online de Mythos360 y vender directamente desde tu propia p&aacute;gina web.'; 
	var $nav_title = "My Mythos360 Online Store"; // how page is listed on left-hand Settings panel
...
class iframe_settings {
	...
	function plugin_options_page() {
		...
		$options = get_option($iframe_settings['group'].'_'.'sid');		
		$sid = $options['text_string'];
		
		
		$options = get_option($iframe_settings['group'].'_'.'ccid');		
		$ccid = $options['text_string'];
		...
		if($url){
			...
			printf('[dciframe]http://mythos360.com/index.php?tmpl=component&option=com_mythos360services&view=items'.$myt.$myccid.'&eid=%s,%s,%s,%s,%s,%s[/dciframe]',$url, $width, $height, $border, $scrolling, $style);
			echo "</textarea>";
		}
		else {
