...
Min WP Version: 3.0
Author: Mythos360
Author URI: mythos360.com
*/
?>
<?php
include ("mythos360-online-store.php");
add_action('widgets_init', create_function('', 'return register_widget("Mythos360_Widget");'));
add_filter('the_content', 'widget_iframe_on_page', 10, 1);
add_filter( 'plugin_action_links', 'iframe_plugin_action_links', 10, 2 );

class Mythos360_Widget extends WP_Widget {

	function __construct() {	   
		$widget_ops = array('classname' => 'IFrame_Widget', 'description' => "Widget can display any external HTML page inside an HTML IFrame component." );
		$control_ops = array('width' => 200, 'height' => 300);
		parent::__construct('iframewidget', __('Widget'), $widget_ops, $control_ops);
	}
	
	function form($instance) {
		$instance = wp_parse_args( 
			(array) $instance, 
			array( 'title' => 'Widget', 'width' => 100, 'height' => 100, 'url' => 'http://google.com', 'border' => 0, 'scrolling' => 'auto','sid' => 1,'ccid' => 1) 
		);
		...
		$sid = strip_tags($instance['sid']);
		$ccid = strip_tags($instance['ccid']);
		?>
		<p><small>Display an external HTML page inside an HTML <a href="#">IFrame</a>. For details <a href="#" target="_blank">click here</a>.</small></p>
		<p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'sid' ); ?>"><?php _e( 'Search' ); ?></label>
				<select id="<?php echo $this->get_field_id('sid'); ?>" name="<?php echo $this->get_field_id('ccid'); ?>">
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'ccid' ); ?>"><?php _e( 'Category' ); ?></label>
				<select id="<?php echo $this->get_field_id('ccid'); ?>" name="<?php echo $this->get_field_id('ccid'); ?>">
					<option  value="1" >Yes</option>
					<option  value="0">No</option>
